...
class Trainer:
    ...
    def set_model(self):
        ...
        w_fc2 = self.weight_variable([1024, 70])
        b_fc2 = self.bias_variable([70])

        # weight & bias matrix depends on features
        w_fc2_len = self.weight_variable([1024, 10])
        b_fc2_len = self.bias_variable([10])

        w_fc2_d1 = self.weight_variable([1024, 10])
        b_fc2_d1 = self.bias_variable([10])

        w_fc2_d2 = self.weight_variable([1024, 10])
        b_fc2_d2 = self.bias_variable([10])

        w_fc2_d3 = self.weight_variable([1024, 10])
        b_fc2_d3 = self.bias_variable([10])

        w_fc2_d4 = self.weight_variable([1024, 10])
        b_fc2_d4 = self.bias_variable([10])

        w_fc2_d5 = self.weight_variable([1024, 10])
        b_fc2_d5 = self.bias_variable([10])

        w_fc2_d6 = self.weight_variable([1024, 10])
        ...
        # readout layer
        pred_r_len = tf.matmul(h_fc1_dropout, w_fc2_len) + b_fc2_len
        pred_len = tf.reshape(pred_r_len, [-1,10])

        pred_r_d1 = tf.matmul(h_fc1_dropout, w_fc2_d1) + b_fc2_d1
        pred_d1 = tf.reshape(pred_r_d1, [-1,10])

        pred_r_d2 = tf.matmul(h_fc1_dropout, w_fc2_d2) + b_fc2_d2
        pred_d2 = tf.reshape(pred_r_d2, [-1,10])

        pred_r_d3 = tf.matmul(h_fc1_dropout, w_fc2_d3) + b_fc2_d3
        pred_d3 = tf.reshape(pred_r_d3, [-1,10])

        pred_r_d4 = tf.matmul(h_fc1_dropout, w_fc2_d4) + b_fc2_d4
        pred_d4 = tf.reshape(pred_r_d4, [-1,10])

        pred_r_d5 = tf.matmul(h_fc1_dropout, w_fc2_d5) + b_fc2_d5
        pred_d5 = tf.reshape(pred_r_d5, [-1,10])

        pred_r_d6 = tf.matmul(h_fc1_dropout, w_fc2_d6) + b_fc2_d6
        ...
        predict = tf.reshape(tf.stack([pred_len, pred_d1, pred_d2, pred_d3, pred_d4, pred_d5, pred_d6], axis=1), (-1, 70))
        print(predict)
...
def train_for_mnist_synthetic():
    ...
    trainer = Trainer([None, 64, 64, 1], [None, 70])
