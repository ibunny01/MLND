...
# bboxHelper handles the coding difference when there is exactly one bbox or an array of bbox.
    ...
    def bboxHelper(self, attr):
        if (len(attr) > 1):
            attr = [
                self.inf[attr.value[j].item()].value[0][0]
...
# getAllDigitStructure returns all the digitStruct from the input file.
    ...
    def getAllDigitStructure(self):
        return [
            self.getDigitStructure(i) for i in range(
                len(self.digitStructName))
        ]
...
def main():
    ...
    parser.add_option(
        "-f",
        dest="fin",
        help="Matlab full number SVHN input file",
        default="digitStruct.mat")
    parser.add_option(
        "-o",
        dest="filePrefix",
        help="name for the json output file",
        default="digitStruct")
