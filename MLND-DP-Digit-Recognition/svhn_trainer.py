...
from svhn_data_loader import SVHNLoader
...
class SVHNTrainer(CNNTrainer):
    ...
    def set_model(self):
        ...
        with tf.name_scope('fully_connected_layer1') as fc_scope1:
            ...
            h_pool_flat = tf.reshape(h_pool4, [-1, shape[1] * shape[2] * shape[3]])

            h_pool_flat_dropout = tf.nn.dropout(h_pool_flat, self.tf_keep_prob, seed=self.seed)
            h_fc1 = tf.nn.relu(tf.matmul(h_pool_flat_dropout, w_fc1) + b_fc1)
        ...
        with tf.name_scope('training') as tr_scope:
            self.tf_optimizer = tf.train.AdamOptimizer(learning_rate=self.tf_learning_rate,
                                                       beta1=0.9, beta2=0.999, epsilon=1e-08,
                                                       use_locking=False, name='Adam').minimize(self.tf_loss)
